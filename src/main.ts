...
import {GlobalLoaderManager} from './Loader'
...
export default class ObsidianLiveSyncPlugin extends Plugin implements LiveSyncLocalDBEnv, LiveSyncReplicatorEnv {
    ...
    registerContext<T>(k: Class<T>, v: T) {
    ...
    > {
        ...
        if (auth.username && auth.password) {
            const utf8str = String.fromCharCode.apply(
                null,
        ...
        try {
            if (this.isRedFlagRaised() || this.isRedFlag2Raised() || this.isRedFlag3Raised()) {
                ...
                if (this.isRedFlag2Raised()) {
                    Logger(
                        `${FLAGMD_REDFLAG2} has been detected! Self-hosted LiveSync suspends all sync and rebuild everything.`,
                        LOG_LEVEL.NOTICE
                    )
                    ...
                    if (
                        (await askYesNo(
                            this.app,
                            'Do you want to disable Suspend file watching and restart obsidian now?'
                        )) == 'yes'
                    ) {
                    ...
                    Logger(
                        `${FLAGMD_REDFLAG3} has been detected! Self-hosted LiveSync will discard the local database and fetch everything from the remote once again.`,
                        LOG_LEVEL.NOTICE
                    )
                ...
                if (this.settings.suspendFileWatching) {
                    Logger(
                        "'Suspend file watching' turned on. Are you sure this is what you intended? Every modification on the vault will be ignored.",
                        LOG_LEVEL.NOTICE
                    )
                }
    ...
    async onload() {
        await GlobalLoaderManager.onloadAll(this)
        // this.get(SyncTaskManager)
    }
                ...
                if (queue.type == 'DELETE') {
                    ...
                    if (queue.type == 'CREATE' || queue.type == 'CHANGED') {
                        ...
                        if (!(await this.updateIntoDB(targetFile, false, cache))) {
                            Logger(
                                `DB -> STORAGE: failed, cancel the relative operations: ${targetFile.path}`,
                                LOG_LEVEL.INFO
                            )
            ...
            if (this.settings.writeDocumentsIfConflicted) {
                ...
                if (d && !d._conflicts) {
                    ...
                    if (!queueConflictCheck()) {
                        Logger(
                            `${this.getPath(change)} is conflicted, write to the storage has been pended.`,
                            LOG_LEVEL.NOTICE
                        )
                    }
                }
    ...
    queuedFilesStore = getGlobalStore('queuedFiles', {
        queuedItems: [] as queueItem[],
        fileEventItems: [] as FileEventItem[],
    })
    ...
    async loadQueuedFiles() {
        ...
        for (const doc of ret.rows) {
            if (doc.doc && !this.queuedFiles.some((e) => e.entry._id == doc.doc._id)) {
                await this.parseIncomingDoc(
                    doc.doc as PouchDB.Core.ExistingDocument<EntryBody & PouchDB.Core.AllDocsMeta>
                )
            }
        }
    }
    ...
    procQueuedFiles() {
        ...
        for (const queue of this.queuedFiles) {
            ...
            if (queue.missingChildren.length == 0) {
                ...
                if (!queue.warned)
                    Logger(
                        `Timed out: ${queue.entry._id} could not collect ${queue.missingChildren.length} chunks. plugin keeps watching, but you have to check the file after the replication.`,
                        LOG_LEVEL.NOTICE
                    )
                queue.warned = true
                continue
    ...
    async parseIncomingDoc(doc: PouchDB.Core.ExistingDocument<EntryBody>) {
        ...
        const ignoreFiles = ['_design/replicate', FLAGMD_REDFLAG, FLAGMD_REDFLAG2, FLAGMD_REDFLAG3]
    ...
    async parseReplicationResult(docs: Array<PouchDB.Core.ExistingDocument<EntryDoc>>): Promise<void> {
        ...
        L1: for (const change of docsSorted) {
            ...
            if (
                change.type != 'leaf' &&
                change.type != 'versioninfo' &&
                change.type != 'milestoneinfo' &&
                change.type != 'nodeinfo'
            ) {
                    ...
                    ? '\nPending: ' +
                      Object.entries(
                          e.pending.reduce(
                              (p, c) => ({
                                  ...p,
                                  [getProcKind(c)]: (p[getProcKind(c)] ?? 0) + 1,
                              }),
...
`
                    ...
                    const ret = await confirmWithMessage(
                        this,
                        'Locked',
                        message,
                        [CHOICE_CLEANUP, CHOICE_FETCH, CHOICE_DISMISS],
                        CHOICE_DISMISS,
                        10
                    )
...
Or if you are sure know what had been happened, we can unlock the database from the setting dialog.
    ...
    async syncAllFiles(showingNotice?: boolean) {
        ...
        for await (const doc of this.localDatabase.findAllNormalDocs()) {
            ...
            if (count % 25 == 0)
                Logger(
                    ...
                    showingNotice ? LOG_LEVEL.NOTICE : LOG_LEVEL.INFO,
                    'syncAll'
                )
        ...
        if (!initialScan) {
            ...
            caches =
                (await this.kvDB.get<{
                    [key: string]: { storageMtime: number; docMtime: number }
                }>('diff-caches')) || {}

            const syncFilesBatch = [...arrayToChunkedArray(syncFiles, 100)]
            const processes = syncFilesBatch.map((files, idx, total) => async () => {
                const dbEntries = await mapAllTasksWithConcurrencyLimit(
                    10,
                    files.map((file) => async () => ({
                        file: file,
                        id: await this.path2id(getPathFromTFile(file)),
                    }))
                )
                const dbEntriesOk = dbEntries.map((e) => ('ok' in e ? e.ok : undefined)).filter((e) => e)
                const docs = await this.localDatabase.allDocsRaw<EntryDoc>({
                    keys: dbEntriesOk.map((e) => e.id),
                    include_docs: true,
                })
                const docsMap = docs.rows.reduce(
                    (p, c) => ({ ...p, [c.id]: c.doc }),
                    {} as Record<DocumentID, EntryDoc>
                )
    ...
    async getConflictedStatus(path: FilePathWithPrefix): Promise<diff_check_result> {
        ...
        if (
            (isSensibleMargeApplicable(path) || isObjectMargeApplicable(path)) &&
            ...
            () =>
                new Promise((res, rej) => {
                    ...
                    new ConflictResolveModal(this.app, filename, conflictCheckResult, async (selected) => {
                        const testDoc = await this.localDatabase.getDBEntry(
                            filename,
                            { conflicts: true },
                            false,
                            false,
                        ...
                        const toKeep =
                            conflictCheckResult.left.rev != toDelete
                                ? conflictCheckResult.left.rev
                                : conflictCheckResult.right.rev
                        if (toDelete == '') {
                            // concat both,
    ...
    filterTargetFiles(files: InternalFileInfo[], targetFiles: string[] | false = false) {
        ...
        return files
            .filter((file) => !ignorePatterns.some((e) => file.path.match(e)))
